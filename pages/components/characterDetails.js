...
export default function CharacterDetails({ data, nameChamp }) {
  ...
  const [imgSplash, setImgSplash] = useState(
    "../../images/centered/GENERIC.png"
  );
  ...
  function getNumericFromString(stringDamage) {
    ...
    let scope = {
      ...
      MOD_Heal:
        ...
        itemStats["HEAL"] +
        runeStats["Heal"] +
        gameStats["Chemtech"] * 0.06, // Revitalize
      MOD_Hit: 1,
      MOD_Magic: 1,
      MOD_OH: 1,
      MOD_Phys: 1,
      ...
      OH_Magic:
        (itemStats["MOH"] +
          (bonusStats["Ardent"] ? 15 + (15 * (level - 1)) / 17 : 0)) *
        1,
      OH_True:
        (nameChamp.toLowerCase().includes("master") &&
        eSkillPoint > 0 &&
        steroidStats["E"]
          ? 25 + 5 * eSkillPoint + 0.3 * additionnalStats["Attack Damage"]
          : 0) +
        (nameChamp.toLowerCase().includes("belveth") && rSkillPoint > 0
          ? ((3 +
              1.5 +
              rSkillPoint +
              0.09 * additionnalStats["Attack Damage"]) *
              stackDarkHarvest) /
            2
          : 0),
      ...
      Self_BoMS:
        additionnalStats["Move Speed"] - basicStatsChampion["Move Speed"],
  ...
  const enemyDataPrep = async () => {
    ...
    if (enemy_obj["Name"] != "-") {
      ...
      enemy_obj["Armor"] =
        championDetails["AR"] + championDetails["AR+"] * (enemyLevel - 1);
      enemy_obj["Hp"] =
        championDetails["HP"] + championDetails["HP+"] * (enemyLevel - 1);
      enemy_obj["Magic Resist"] =
        championDetails["MR"] + championDetails["MR+"] * (enemyLevel - 1);
    }
  ...
  const [stackLegendExceptBloodline, setStackLegendExceptBloodline] =
    useState(0);
  ...
  const majStatsRune = () => {
    ...
    if (
      mainSecondRune == "Biscuit Delivery" ||
      secondFirstRune == "Biscuit Delivery" ||
      secondSecondRune == "Biscuit Delivery"
    ) {
      if (Math.floor(gameStats["Gametime"] / 2) * 40 >= 120) obj["MP"] += 120;
      else obj["MP"] += Math.floor(gameStats["Gametime"] / 2) * 40;
    }
    if (
      mainFirstRune == "Manaflow Band" ||
      secondFirstRune == "Manaflow Band"
    ) {
      obj["MP"] += 250;
    }
    if (mixedShard == "Armor") {
    ...
    if (
      mainSecondRune == "Conditioning" ||
      secondFirstRune == "Conditioning" ||
      secondSecondRune == "Conditioning"
    ) {
    ...
    if (
      mainThirdRune == "Ultimate Hunter" ||
      secondSecondRune == "Ultimate Hunter"
    ) {
      obj["Ultimate"] = 6 + 5 * stackBounty;
    }
    setRuneStats(obj);
  };
  ...
  const majItemStats = async () => {
    ...
    setHasChalice(returnBoolIfHasItem("Chalice of Blessing"));
    setHasEchoes(returnBoolIfHasItem("Echoes of Helia"));
    setHasMikael(returnBoolIfHasItem("Mikael's Blessing"));
    setHasRedemption(returnBoolIfHasItem("Redemption"));
    setHasSpectre(returnBoolIfHasItem("Spectre's Cowl"));
    setHasWarmog(returnBoolIfHasItem("Warmog"));
    setHasArchangel(returnBoolIfHasItem("Archangel's Staff"));
    setHasDemonic(returnBoolIfHasItem("Demonic Embrace"));
    setHasEverfrost(returnBoolIfHasItem("Everfrost"));
    setHasRabadon(returnBoolIfHasItem("Rabadon's Deathcap"));
    setHasRiftmaker(returnBoolIfHasItem("Riftmaker"));
    setHasSeraph(returnBoolIfHasItem("Riftmaker"));
    setHasFlowing(returnBoolIfHasItem("Staff of Flowing Water"));
    setHasRapidFireCanon(returnBoolIfHasItem("Rapid Firecannon"));
    setHasDivine(returnBoolIfHasItem("Divine Sunderer"));
  ...
  function returnBoolIfHasItem(chaine) {
    ...
    if (
      itemSlot2.includes(chaine) ||
      itemSlot3.includes(chaine) ||
      itemSlot4.includes(chaine) ||
      itemSlot5.includes(chaine) ||
      itemSlot6.includes(chaine)
    )
      bool = true;
    if (elixirSlot.includes(chaine)) bool = true;
    return bool;
  }
    ...
    if (data != undefined) {
      ...
      let champ_obj = {
        Hp:
          data["HP"] +
          data["HP+"] * (level - 1) +
          (steroidStats["Form"] && nameChamp == "Gnar"
            ? 100 + 43 * (level - 1)
            : 0) +
          (steroidStats["Form"] && nameChamp == "Kled"
            ? 810 + 144 * (level - 1) * (0.007025 + 0.000175 * (level - 1))
            : 0),
        "Attack Damage":
          (data["AD"] + data["AD+"] * (level - 1)) *
          (1 +
            (steroidStats["Items"] && hasTrinity ? 0.2 : 0) +
            (steroidStats["Form"] && nameChamp == "Gnar"
              ? 8 + 2.5 * (level - 1)
              : 0)),
        "Attack Speed %": Number(
          data["AS"] * (1 + (data["Ratio"] * (level - 1)) / 100)
        ),
        Armor:
          data["AR"] +
          data["AR+"] * (level - 1) +
  ...
  useEffect(() => {
    ...
    // SE referer au WIKI
    let bonusAD =
      (hasBloodthirster && steroidStats["Items"]
        ? level < 13
          ? 10
          : level < 14
          ? 15
          ...
          : 40
        : 0) +
      (hasSterak ? basicStatsChampion["Attack Damage"] / 2 : 0) +
      (hasGaleforce || hasIE || hasNavori ? 5 * nbLegendary : 0) +
      (hasRavenousHydra && steroidStats["Items"] ? 20 : 0) +
      (hasTrinity ? 3 * nbLegendary : 0) +
      (hasYoumuu ? 7 * nbLegendary : 0) +
      (hasTitanicHydra ? 0.02 * additionnalStats["Hp"] : 0);
    bonusAD +=
      nameChamp == "Aphelios"
        ? qSkillPoint == 0
          ? 4.5
        ...
        : 0;
    bonusAD +=
      nameChamp == "Darius" && steroidStats["P"]
        ? level <= 10
          ? 30 + 5 * (level - 1)
          : level <= 13
          ? 30 + 10 * (level - 1)
        ...
        : 0;
    bonusAD +=
      nameChamp.includes("Mundo") && eSkillPoint > 0
        ? 2 + 0.5 * eSkillPoint
        : 0;
    bonusAD +=
      nameChamp == "Hecarim"
        ? additionnalStats["Move Speed"] *
          (0.12 + (level >= 3 ? 0.02 * Math.floor((level - 3) / 3) : 0))
        : 0;
    bonusAD +=
      nameChamp.includes("Sante") && steroidStats["R"]
        ? 5 +
          additionnalStats["Armor"] * 0.325 +
          additionnalStats["Magic Resist"] * 0.325
        : 0;
    bonusAD +=
      nameChamp == "Nocturne" && steroidStats["Q"] ? 10 + 10 * qSkillPoint : 0;
    bonusAD += nameChamp == "Senna" ? 0.75 * sennaStacks : 0;
    bonusAD +=
      nameChamp == "Tryndamere" && qSkillPoint > 0
        ? 10 + 5 * (qSkillPoint - 1)
        : 0;
    bonusAD +=
      nameChamp == "Twitch" && steroidStats["R"]
        ? 40 + 15 * (rSkillPoint - 1)
        : 0;
    bonusAD +=
      nameChamp == "Vayne" && steroidStats["R"]
        ? 25 + 15 * (rSkillPoint - 1)
        : 0;
    bonusAD += nameChamp == "Pyke" ? 0.07143 * additionnalStats["Hp"] : 0;
    bonusAD +=
      mainRune == "Conqueror" && runeStats["ForceBit"] == 1
        ? (1.2 + 1.5 / (17 * (level - 1))) * stackConqueror
        : 0;
    bonusAD +=
      (secondFirstRune.includes("EyeBall") ||
        secondSecondRune.includes("EyeBall") ||
        mainSecondRune.includes("Eyeball")) &&
      runeStats["ForceBit"] == 1
        ? 1.2 * stackBounty + (stackBounty == 10 ? 6 : 0)
        : 0;
    bonusAD +=
      (secondFirstRune.includes("Poro") ||
        secondSecondRune.includes("Poro") ||
        mainSecondRune.includes("Poro")) &&
      runeStats["ForceBit"] == 1
        ? 1.2 * stackBounty + (stackBounty == 10 ? 6 : 0)
        : 0;
    bonusAD +=
      (secondFirstRune.includes("Zombie") ||
        secondSecondRune.includes("Zombie") ||
        mainSecondRune.includes("Zombie")) &&
      runeStats["ForceBit"] == 1
        ? 1.2 * stackBounty + (stackBounty == 10 ? 6 : 0)
        : 0;
    bonusAD +=
      (secondFirstRune.includes("Absolute Focus") ||
        secondSecondRune.includes("Absolute Focus") ||
        mainSecondRune.includes("EyeAbsolute Focusball")) &&
      runeStats["ForceBit"] == 1
        ? 1.8 + 16.2 / (17 * (level - 1))
        : 0;
    bonusAD +=
      (secondFirstRune.includes("Gathering Storm") ||
        secondSecondRune.includes("Gathering Storm") ||
        mainSecondRune.includes("Gathering Storm")) &&
      runeStats["ForceBit"] == 1
        ? gameStats["Gametime"] < 10
          ? 0
          : gameStats["Gametime"] < 20
          ? 4.8
          : gameStats["Gametime"] < 30
          ? 14.4
          : gameStats["Gametime"] < 40
          ? 28.8
          : 48
        : 0;
    ...
    obj["Attack Damage"] =
      (itemStats["AD"] +
        (runeStats["ForceBit"] == 1 ? runeStats["Adaptive"] : 0) +
        bonusAD) *
      (1 +
        0.05 * gameStats["Infernal"] +
        (hasWardStone ? 0.2 : 0) +
        (nameChamp == "Riven" && steroidStats["R"] ? 0.2 : 0) +
        (nameChamp == "Rengar" && steroidStats["P"] ? 0.25 : 0));

    // HP
    let bonusHP = hasGoredrinker || hasRadiant ? 75 * nbLegendary : 0;
    bonusHP += hasIceborn ? 50 * nbLegendary : 0;
    bonusHP += nameChamp.includes("Gath")
      ? 40 + 40 * rSkillPoint * gameStats["Kills"]
      : 0;
    bonusHP +=
      nameChamp == "Nasus" && steroidStats["R"] ? 150 + 150 * rSkillPoint : 0;
    bonusHP +=
      nameChamp == "Renekton" && steroidStats["R"]
        ? 100 + 150 * rSkillPoint
        : 0;
    bonusHP +=
      nameChamp == "Shyvana" && steroidStats["R"] ? 50 + 100 * rSkillPoint : 0;
    bonusHP +=
      nameChamp == "Sion" && wSkillPoint > 0
        ? 4 * gameStats["Minion"] + 15 * gameStats["Kills"]
        : 0;
    bonusHP += nameChamp == "Swain" ? 12 * gameStats["Minion"] : 0;
    bonusHP +=
      nameChamp == "Volibear" && steroidStats["R"] ? 0 + 175 * rSkillPoint : 0;
    bonusHP += stackLegendBloodline == 15 ? 85 : 0;
    bonusHP +=
      mainThirdRune == "Overgrowth" || secondSecondRune == "Overgrowth"
        ? 3 * Math.floor(gameStats["Minion"] / 8) >= 45
          ? 0.035 * basicStatsChampion["Hp"]
          : 0
        : 0;
    bonusHP +=
      mainThirdRune == "Overgrowth" || secondSecondRune == "Overgrowth"
        ? 3 * Math.floor(gameStats["Minion"] / 8)
        : 0;
    let multiBonusHp =
      mainThirdRune == "Overgrowth" || secondSecondRune == "Overgrowth"
        ? 3 * Math.floor(gameStats["Minion"] / 8) >= 45
    ...
    multiBonusHp *=
      nameChamp == "Ornn"
        ? Math.min(1.3, 1.1 + (level > 12 ? (level - 12) * 0.04 : 0))
        : 1;
    ...
    let bonusAS =
      (bonusStats["Ardent"] ? 0.2 : 0) +
      (hasRageblade && steroidStats["Items"] ? 0.32 : 0) +
      (hasShieldbow && steroidStats["Items"] ? 0.3 : 0) +
      (hasPhantom && steroidStats["Items"] ? 0.3 : 0) +
      (hasRageknife && steroidStats["Items"] ? 0.15 : 0);
    bonusAS +=
      nameChamp.includes("Jarvan") && steroidStats["Q"] && qSkillPoint > 0
        ? 0.175 + 0.025 * qSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Lulu" && steroidStats["W"] && wSkillPoint > 0
        ? 0.225 + 0.025 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Nidalee" && steroidStats["E"] && eSkillPoint > 0
        ? 0.1 + 0.1 * eSkillPoint
        : 0;
    ...
    bonusAS +=
      nameChamp == "Aphelios"
        ...
        : 0;
    bonusAS +=
      nameChamp == "Ashe" && steroidStats["Q"] && qSkillPoint > 0
        ? 0.175 + 0.075 * qSkillPoint
        : 0;
    bonusAS += nameChamp.includes("Veth")
      ? gameStats["Minion"] +
        2 * gameStats["Kills"] * 0.0022 +
        0.0006 * level +
        (steroidStats["P"] ? 0.25 + 0.25 / (17 * level - 1) : 0)
      : 0;
    bonusAS +=
      nameChamp.includes("Blitz") && steroidStats["W"] && wSkillPoint > 0
        ? 0.17 + 0.13 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Camille" && steroidStats["E"] && eSkillPoint > 0
        ? 0.35 + 0.05 * eSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Diana" && steroidStats["P"]
        ? level < 3
          ? 0.15
          : level < 6
          ? 0.1917
        ...
        : 0;
    bonusAS +=
      nameChamp == "Draven" && steroidStats["W"] && wSkillPoint > 0
        ? 0.15 + 0.05 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Elise" &&
      steroidStats["W"] &&
    ...
    bonusAS +=
      nameChamp == "Fiora" && steroidStats["E"] && eSkillPoint > 0
        ? 0.4 + 0.1 * eSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Gnar" && !steroidStats["Form"]
        ? 0.055 + eSkillPoint > 0
          ? 0.35 + 0.05 * eSkillPoint
          : 0
        : 0;
    bonusAS +=
      nameChamp == "Gwen" && steroidStats["E"] && eSkillPoint > 0
        ? 0.05 + 0.15 * eSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Irelia" && steroidStats["P"]
        ? level < 7
          ? 0.3
          : level < 14
          ? 0.55
        ...
        : 0;
    bonusAS +=
      nameChamp == "Jax" && steroidStats["P"]
        ? 0.28 + 0.12 * Math.floor(level / 3)
        : 0;
    bonusAS +=
      nameChamp == "Jinx" && !steroidStats["Q"] && qSkillPoint > 0
        ? 0.05 + 0.25 * qSkillPoint
        : 0;
    bonusAS +=
      ...
      eSkillPoint > 0
        ...
        : 0;
    bonusAS +=
      nameChamp == "Kennen" && steroidStats["E"] && eSkillPoint > 0
        ? 0.3 + 0.1 * eSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Kindred" && steroidStats["Q"]
        ? 0.35 + 0.05 * gameStats["Kills"]
        : 0;
    bonusAS += nameChamp == "Kled" && steroidStats["W"] ? 1.5 : 0;
    bonusAS +=
      nameChamp.includes("Kog") && nameChamp.includes("Maw") && qSkillPoint > 0
        ? 0.05 + 0.05 * qSkillPoint
        : 0;
    bonusAS +=
      ...
      steroidStats["P"]
        ...
        : 0;
    bonusAS +=
      nameChamp.includes("Master") && steroidStats["R"] && rSkillPoint > 0
        ? 0.15 + 0.1 * rSkillPoint
        : 0;
    bonusAS +=
      nameChamp.includes("Fortune") && steroidStats["W"] && wSkillPoint > 0
        ? 0.25 + 0.15 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Nilah" && steroidStats["Q"] && qSkillPoint > 0
        ? 0.1 + 0.5 / (17 * (level - 1))
        : 0;
    bonusAS +=
      nameChamp == "Nocturne" && steroidStats["W"] && wSkillPoint > 0
        ? 0.25 + 0.05 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Olaf" && steroidStats["P"]
        ? 0.5 + (50 / 17) * (level - 1) * (0.007025 + 0.000175 * (level - 1))
        : 0;
    bonusAS +=
      nameChamp == "Olaf" && steroidStats["W"] && wSkillPoint > 0
        ? 0.3 + 0.1 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Qiyana" && steroidStats["W"] ? 0.05 * wSkillPoint : 0;
    bonusAS +=
      nameChamp == "Quinn" && steroidStats["W"] && wSkillPoint > 0
        ? 0.2 + 0.08 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Rammus" && steroidStats["E"] && eSkillPoint > 0
        ? 0.15 + 0.05 * eSkillPoint
        : 0;
    bonusAS += nameChamp == "Rell" && steroidStats["W"] ? 0.3 : 0;
    bonusAS += nameChamp == "Rengar" && steroidStats["Q"] ? 0.4 : 0;
    bonusAS +=
      nameChamp == "Rumble" && steroidStats["P"]
        ? (50 +
            ((130 - 50) / 17) * (level - 1) * (0.7025 + 0.0175 * (level - 1))) /
          100
        : 0;
    bonusAS +=
      nameChamp == "Samira" && steroidStats["E"] && eSkillPoint > 0
        ? 0.15 + 0.05 * eSkillPoint
        : 0;
    bonusAS += nameChamp == "Shen" && steroidStats["Q"] ? 0.5 : 0;
    bonusAS +=
      nameChamp == "Sivir" && steroidStats["W"] && wSkillPoint > 0
        ? 0.15 + 0.05 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Skarner" && steroidStats["P"]
        ? (38 + (level <= 7 ? 5 : level <= 13 ? 7 : 9)) / 100
        : 0;
    bonusAS += nameChamp == "Sylas" && steroidStats["P"] ? 1.25 : 0;
    bonusAS += nameChamp == "Taric" && steroidStats["P"] ? 1 : 0;
    bonusAS +=
      nameChamp == "Teemo" && steroidStats["P"]
        ? level < 5
          ? 0.2
          : level < 10
          ? 0.4
        ...
        : 0;
    bonusAS +=
      nameChamp == "Tristana" && steroidStats["Q"] && qSkillPoint > 0
        ? 0.5 + 0.15 * qSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Trundle" && steroidStats["W"] && wSkillPoint > 0
        ? 0.1 + 0.2 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp.includes("Twisted") && steroidStats["E"] && eSkillPoint > 0
        ? (3.5 + 7.5 * eSkillPoint) / 100
        : 0;
    bonusAS +=
      nameChamp == "Twitch" && steroidStats["Q"] && qSkillPoint > 0
        ? 0.35 + 0.5 * qSkillPoint
        : 0;
    bonusAS += nameChamp == "Udyr" && steroidStats["P"] ? 0.3 : 0;
    bonusAS +=
      nameChamp == "Udyr" && steroidStats["Q"] && qSkillPoint > 0
        ? 0.08 + 0.12 * qSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Varus" && steroidStats["P"]
        ? 0.1 + (level <= 7 ? 0.5 : 0.1) + 0.2
        : 0;
    bonusAS +=
      nameChamp == "Vi" && steroidStats["W"] && wSkillPoint > 0
        ? 0.225 + 0.075 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Viego" && steroidStats["E"] && eSkillPoint > 0
        ? 0.25 + 0.05 * eSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Warwick" && steroidStats["W"] && wSkillPoint > 0
        ? 0.6 + 0.1 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Wukong" && steroidStats["E"] && eSkillPoint > 0
        ? 0.3 + 0.05 * eSkillPoint
        : 0;
    bonusAS +=
      nameChamp == "Xayah" && steroidStats["W"] && wSkillPoint > 0
        ? 0.3 + 0.05 * wSkillPoint
        : 0;
    bonusAS +=
      nameChamp.includes("Zhao") && steroidStats["E"] && eSkillPoint > 0
        ? 0.35 + 0.05 * eSkillPoint
        : 0;
    bonusAS += nameChamp == "Zeri" && steroidStats["R"] ? 0.3 : 0;
    bonusAS += mainRune == "Hail of Blades" && steroidStats["Runes"] ? 1.1 : 0;
    bonusAS +=
      mainRune == "Lethal Tempo" && steroidStats["Runes"]
        ? data["Melee?"] == 1
          ? level >= 15
            ? 0.9
            : 0.57 + 0.03 * level
        ...
        : 0;
    bonusAS +=
      mainSecondRune == "Alacrity" ||
      secondFirstRune == "Alacrity" ||
      secondSecondRune == "Alacrity"
        ? 0.03 + 0.15 * stackLegendExceptBloodline
        : 0;
    bonusAS += 0.075 * gameStats["Hextech"];
    let multiBonusAS =
      (nameChamp == "Jinx" && steroidStats["Q"] && qSkillPoint > 0 ? 0.9 : 1) *
      (nameChamp.includes("Veth") && steroidStats["Form"] && rSkillPoint > 0
        ? 1.05 + 0.05 * rSkillPoint
        : 1);
    obj["Attack Speed %"] =
      (itemStats["AS"] / 100 +
        runeStats["AS"] +
        ...
        (nameChamp == "Jhin"
          ? 0
          : data["AS+"] * (level - 1) * (0.685 + 0.0175 * level))) *
        multiBonusAS +
      (nameChamp == "Varus" && steroidStats["P"]
        ? 0.4 +
          0.4 *
        ...
        : 0);
    obj["Attack Speed %"] *=
      nameChamp == "Jayce" && steroidStats["Form"] && steroidStats["W"] ? 3 : 1;

    // ARMOR
    let bonusArmor = itemSlot1 == "Evenshroud" ? 5 * nbLegendary : 0;
    bonusArmor += hasHullbreaker
      ? data["Melee?"] == 1
        ? level < 12
          ? 30
          : level < 13
    ...
    bonusArmor +=
      nameChamp == "Anivia" && steroidStats["P"]
        ? level < 5
          ? -40
          : level < 8
          ? -25
        ...
        : 0;
    bonusArmor +=
      nameChamp == "Graves" && steroidStats["E"] && eSkillPoint > 0
        ? (1 + 3 * eSkillPoint) * gameStats["Minion"]
        : 0;
    bonusArmor +=
      nameChamp == "Hecarim" && steroidStats["W"] && wSkillPoint > 0
        ? 10 + 5 * wSkillPoint
        : 0;
    bonusArmor +=
      nameChamp == "Jax" && steroidStats["R"] && rSkillPoint > 0
        ? -10 + 25 * rSkillPoint + 0.4 * obj["Attack Damage"]
        : 0;
    bonusArmor +=
      nameChamp == "Jayce" && steroidStats["Form"] && steroidStats["R"]
        ? (level < 6 ? 5 : level < 11 ? 15 : level < 15 ? 25 : 35) +
          0.075 * obj["Attack Damage"]
        : 0;
    bonusArmor +=
      nameChamp == "Kennen" && steroidStats["R"] ? 20 * rSkillPoint : 0;
    bonusArmor +=
      nameChamp == "Nasus" && steroidStats["R"] && rSkillPoint > 0
        ? 25 + 15 * rSkillPoint
        : 0;
    bonusArmor +=
      nameChamp == "Olaf" && steroidStats["R"] ? rSkillPoint * 10 : 0;
    bonusArmor +=
      nameChamp == "Oriana" && eSkillPoint > 0 ? 6 * eSkillPoint : 0;
    bonusArmor +=
      nameChamp == "Shyvana"
        ? 5 *
          (gameStats["Chemtech"] +
        ...
        : 0;
    bonusArmor +=
      nameChamp == "Singed" && steroidStats["R"] && rSkillPoint > 0
        ? -5 + 35 * rSkillPoint
        : 0;
    bonusArmor += nameChamp == "Thresh" ? 1 * gameStats["Minion"] : 0;
    bonusArmor +=
      nameChamp == "Trundle" && steroidStats["R"]
        ? 0.4 * enemyStats["Armor"]
        : 0;
    bonusArmor +=
      nameChamp == "Wukong" && steroidStats["P"]
        ? 30 + (24 / 17) * (level - 1)
        : 0;
    bonusArmor +=
      (mainFirstRune == "Shield Bash" || secondFirstRune == "Shield Bash") &&
      steroidStats["Runes"]
        ? 1 + (9 / 17) * (level - 1)
        : 0;
    ...
    obj["Armor"] =
      (itemStats["AR"] + runeStats["AR"] + bonusArmor) * multiBonusArmor;
    obj["Armor"] +=
      nameChamp == "Braum" && steroidStats["W"] && wSkillPoint > 0
        ? 15 + 5 * wSkillPoint + 0.36 * obj["Armor"]
        : 0;
    obj["Armor"] +=
      nameChamp == "Garen" && steroidStats["W"] ? 30 + 0.1 * obj["Armor"] : 0;
    obj["Armor"] +=
      nameChamp == "Leona" && steroidStats["W"] && wSkillPoint > 0
        ? 15 + 5 * wSkillPoint + 0.2 * obj["Armor"]
        : 0;
    obj["Armor"] +=
      nameChamp == "Poppy" && wSkillPoint > 0 ? 0.12 * obj["Armor"] : 0;
    obj["Armor"] +=
      nameChamp == "Rell" && wSkillPoint > 0 ? 0.12 * obj["Armor"] : 0;
    obj["Armor"] +=
      nameChamp == "Sejuani" && steroidStats["P"] ? 10 + obj["Armor"] / 2 : 0;
    obj["Armor"] += hasGargoyle ? 0.05 * obj["Armor"] : 0;
    obj["Armor"] +=
      mainRune == "Aftershock" && steroidStats["Runes"]
        ? 35 + 0.8 * obj["Armor"]
        : 0;
    obj["Armor"] *=
      (mainSecondRune == "Conditioning" ||
    ...
    bonusMR +=
      nameChamp == "Jax" && steroidStats["R"] && rSkillPoint > 0
        ? -6 + 25 * rSkillPoint + 0.24 * obj["Attack Damage"]
        : 0;
    bonusMR +=
      nameChamp == "Jayce" && steroidStats["Form"] && steroidStats["R"]
        ? (level < 6 ? 5 : level < 11 ? 15 : level < 15 ? 25 : 35) +
    ...
    let multiBonusMR =
      nameChamp == "Ornn"
        ...
        : 1;
    obj["Magic Resist"] =
      (itemStats["MR"] + runeStats["MR"] + bonusMR) * multiBonusMR;
    obj["Magic Resist"] += hasGargoyle ? 0.05 * obj["Magic Resist"] : 0;
    obj["Magic Resist"] +=
      nameChamp == "Braum" && steroidStats["W"] && wSkillPoint > 0
        ? 15 + 5 * wSkillPoint + 0.36 * obj["Magic Resist"]
        : 0;
    obj["Magic Resist"] +=
      nameChamp == "Garen" && steroidStats["W"]
        ? 30 + 0.1 * obj["Magic Resist"]
        : 0;
    obj["Magic Resist"] +=
      nameChamp == "Leona" && steroidStats["W"] && wSkillPoint > 0
        ? 15 + 5 * wSkillPoint + 0.2 * obj["Magic Resist"]
        : 0;
    obj["Magic Resist"] +=
      nameChamp == "Poppy" && wSkillPoint > 0 ? 0.12 * obj["Magic Resist"] : 0;
    obj["Magic Resist"] +=
      nameChamp == "Rell" && wSkillPoint > 0 ? 0.12 * obj["Magic Resist"] : 0;
    obj["Magic Resist"] +=
      nameChamp == "Sejuani" && steroidStats["P"]
    ...
    bonusMS +=
      hasHearthbound && steroidStats["Items"]
        ? data["Melee?"] == 1
          ? 20
          : 10
        : 0;
    ...
    bonusMS +=
      nameChamp == "Akshan" && steroidStats["W"] && wSkillPoint > 0
        ? 70 + 10 * wSkillPoint
        : 0;
    bonusMS +=
      nameChamp.includes("Veth") && steroidStats["R"] ? 25 * rSkillPoint : 0;
    bonusMS += nameChamp == "Cassiopeia" ? 4 * level : 0;
    bonusMS +=
      nameChamp == "Cassiopeia" && steroidStats["Q"] && qSkillPoint > 0
        ? 25 + 5 * qSkillPoint
        : 0;
    bonusMS += nameChamp == "Elise" && steroidStats["Form"] ? 25 : 0;
    bonusMS += nameChamp == "Jayce" && steroidStats["Form"] ? 40 : 0;
    bonusMS +=
      nameChamp == "Lucian" && steroidStats["W"] && wSkillPoint > 0
        ? 50 + 5 * wSkillPoint
        : 0;
    bonusMS +=
      nameChamp == "Akshan" && steroidStats["P"]
        ? (20 + (55 / 17) * (level - 1)) * (1 + obj["Attack Speed %"])
        : 0;
    bonusMS +=
      nameChamp.includes("Fortune") && steroidStats["W"] && wSkillPoint > 0
        ? 45 + 10 * wSkillPoint
    ...
    bonusMS +=
      ...
      steroidStats["Form"]
        ? 15 + (level < 6 ? 0 : level < 11 ? 5 : level < 16 ? 10 : 15)
        : 0;
    bonusMS +=
      nameChamp == "Singed" && steroidStats["R"] && rSkillPoint > 0
        ? -5 + 35 * rSkillPoint
        : 0;
    bonusMS +=
      nameChamp == "Sivir" && steroidStats["P"]
        ? 55 +
          (level < 6
            ? 0
            : level < 11
    ...
    bonusMS +=
      nameChamp == "Skarner" && steroidStats["P"]
        ...
        : 0;
    bonusMS +=
      nameChamp == "Vayne" && steroidStats["P"]
        ? 45 * (steroidStats["R"] ? 2 : 1)
        : 0;
    bonusMS += nameChamp == "Kled" && steroidStats["R"] ? 650 : 0;
    bonusMS +=
      nameChamp == "Rell" && eSkillPoint > 0
        ? level <= 6
          ? 5 + 3 * (level - 1)
          : level <= 11
          ? 20 + 4 * (level - 6)
        ...
        : 1;
    bonusMS +=
      mainThirdRune == "Relentless Hunter" ||
      secondSecondRune == "Relentless Hunter"
        ? 5 + 8 * stackBounty
        : 0;
    bonusMS +=
      mainFirstRune == "Magical Footwear" ||
      secondFirstRune == "Magical Footwear"
        ? 10
        : 0;
    bonusMS +=
      (mainThirdRune == "Waterwalking" || secondSecondRune == "Waterwalking") &&
      steroidStats["Runes"]
        ? 25
        : 0;
    ...
    obj["Move Speed"] =
      ...
      runeStats["MS"] +
      bonusMS +
      basicStatsChampion["Move Speed"];
    console.log(obj["Move Speed"]);
    obj["Move Speed"] *= hasStridebreaker ? 1 + 0.02 * nbLegendary : 1;
    obj["Move Speed"] *=
      nameChamp == "Aatrox" && steroidStats["R"] && rSkillPoint > 0
        ? 1.4 + 0.2 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Ahri" && steroidStats["W"] && wSkillPoint > 0 ? 1.4 : 1;
    obj["Move Speed"] *=
      nameChamp == "Akali" && steroidStats["P"]
        ? 1.3 + (level < 6 ? 0 : level < 11 ? 0.1 : level < 16 ? 0.2 : 0.3)
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Akali" && steroidStats["W"] && wSkillPoint > 0
        ? 1.25 + 0.05 * wSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Bard" && steroidStats["P"] ? 1.24 : 1;
    obj["Move Speed"] *= nameChamp == "Bard" && steroidStats["W"] ? 1.3 : 1;
    obj["Move Speed"] *=
      nameChamp == "Annie" && steroidStats["E"]
        ? 1.2 + (0.3 / 17) * (level - 1)
        : 1;
    obj["Move Speed"] *= nameChamp == "Janna" && steroidStats["P"] ? 1.06 : 1;
    obj["Move Speed"] *=
      nameChamp == "Jayce" && steroidStats["Form"] && steroidStats["E"]
        ? 1.25 + 0.05 * eSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Karma" && steroidStats["E"] ? 1.4 : 1;
    obj["Move Speed"] *=
      nameChamp == "Milio" && steroidStats["E"] && eSkillPoint > 0
        ? 1.125 + 0.025 * eSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Naafiri" && steroidStats["R"] && rSkillPoint > 0
        ? 1.275 + 0.075 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Nilah" && steroidStats["W"] && wSkillPoint > 0
        ? 1.125 + 0.025 * wSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Oriana" && steroidStats["W"] && wSkillPoint > 0
        ? 1.25 + 0.05 * wSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp.includes("Nunu") && steroidStats["P"] ? 1.1 : 1;
    obj["Move Speed"] *=
      nameChamp == "Sivir" && steroidStats["R"] && rSkillPoint
        ? 1.15 + 0.05 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Soraka" && steroidStats["Q"] && qSkillPoint
        ? 1.175 + 0.025 * qSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Yuumi" && steroidStats["E"] ? 1.2 : 1;
    obj["Move Speed"] *=
      nameChamp == "Zilean" && steroidStats["E"] && eSkillPoint > 0
        ? 1.25 + 0.15 * eSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Blitzcrank" && steroidStats["W"] && wSkillPoint > 0
        ? 1.65 + 0.05 * wSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Braum" && steroidStats["E"] ? 1.1 : 1;
    obj["Move Speed"] *=
      nameChamp == "Camille" && steroidStats["Q"] && qSkillPoint > 0
        ? 1.15 + 0.05 * qSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Corki" && steroidStats["P"] ? 1.4 : 1;
    obj["Move Speed"] *=
      nameChamp.includes("Mundo") && steroidStats["R"] && rSkillPoint > 0
        ? 1.05 + 0.1 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Draven" && steroidStats["W"] && wSkillPoint > 0
        ? 1.45 + 0.05 * wSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Ekko" && steroidStats["P"]
        ? 1.5 + 0.1 * (level < 6 ? 0 : level < 11 ? 1 : level < 16 ? 2 : 3)
        : 1;
    obj["Move Speed"] *= nameChamp == "Evelynn" && steroidStats["E"] ? 1.3 : 1;
    obj["Move Speed"] *=
      nameChamp == "Fiora" && steroidStats["P"] ? 1.2 + 0.1 * rSkillPoint : 1;
    obj["Move Speed"] *=
      nameChamp == "Fiora" && steroidStats["R"] ? 1 + 0.1 * rSkillPoint : 1;
    obj["Move Speed"] *=
      nameChamp == "Gankplank" && steroidStats["P"]
        ? 1.15 + (0.15 / 17) * (level - 1)
        : 1;
    obj["Move Speed"] *= nameChamp == "Garen" && steroidStats["Q"] ? 1.35 : 1;
    obj["Move Speed"] *=
      nameChamp == "Gnar" && steroidStats["P"] && !steroidStats["Form"]
        ? 1.2 + 0.2 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Heimerdinger" && steroidStats["P"] ? 1.2 : 1;
    obj["Move Speed"] *=
      nameChamp == "Jhin"
    ...
    obj["Move Speed"] *=
      ...
      steroidStats["E"] &&
      eSkillPoint > 0
        ? (1.5 + 0.05 * eSkillPoint) *
          (1 + 0.01 * Math.floor(obj["Attack Speed %"] / 100))
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Katarina" && wSkillPoint > 0 ? 1.4 + 0.1 * wSkillPoint : 1;
    ...
    obj["Move Speed"] *=
      ...
      steroidStats["P"]
        ...
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Malphite" && steroidStats["Q"] && qSkillPoint > 0
        ? 1.15 + 0.05 * qSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Maokai" && steroidStats["R"] && rSkillPoint > 0
        ? 1.3 + 0.1 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp.includes("Master") && steroidStats["R"] && rSkillPoint > 0
        ? 1.25 + 0.1 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Mordekaiser" && steroidStats["P"]
        ? level < 6
          ? 1.03
          : level < 11
          ? 1.06
        ...
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Morgana" && steroidStats["R"] && rSkillPoint > 0
        ? 0.8 + 0.25 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Neeko" && steroidStats["W"] && wSkillPoint > 0
        ? 1.15 + 0.05 * wSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Nidalee" && steroidStats["P"] ? 1.3 : 1;
    obj["Move Speed"] *= nameChamp == "Nocturne" && steroidStats["E"] ? 1.9 : 1;
    obj["Move Speed"] *=
      nameChamp == "Nocturne" && steroidStats["Q"] && qSkillPoint > 0
        ? 1.1 + 0.05 * qSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Olaf" && steroidStats["R"] && rSkillPoint > 0
        ? 0.95 + 0.25 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Pantheon" && steroidStats["P"] && steroidStats["E"]
        ? 1.6
        : 1;
    obj["Move Speed"] *= nameChamp == "Poppy" && steroidStats["W"] ? 1.4 : 1;
    obj["Move Speed"] *=
      nameChamp == "Quinn" && wSkillPoint > 0 ? 1.15 + 0.05 * wSkillPoint : 1;
    obj["Move Speed"] *= nameChamp == "Rakan" && steroidStats["R"] ? 1.75 : 1;
    obj["Move Speed"] *=
      nameChamp == "Rell" && steroidStats["Form"] && steroidStats["W"]
        ? 1.3
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Rell" && steroidStats["E"] && eSkillPoint > 0
        ? 1.25 + 0.05 * eSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Rengar" && steroidStats["P"]
        ? level < 7
          ? 1.3
          : level < 13
          ? 1.4
        ...
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Rengar" && steroidStats["R"] && rSkillPoint > 0
        ? 1.3 + 0.1 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Rumble" && steroidStats["W"] && wSkillPoint > 0
        ? 1.05 + 0.05 * wSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Ryze" && steroidStats["Q"] && qSkillPoint > 0
        ? 1.24 + 0.04 * qSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Samira" && steroidStats["P"]
        ? 1.12 + 0.03 * (level < 6 ? 0 : level < 11 ? 1 : level < 16 ? 2 : 3)
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Senna" && steroidStats["P"]
        ? 1.1 + (level < 6 ? 0 : level < 9 ? 0.05 : 0.1)
        : 1;
    ...
    obj["Move Speed"] *=
      nameChamp == "Skarner" && steroidStats["W"]
        ? 1.16 + 0.04 * wSkillPoint
        : 1;
    ...
    obj["Move Speed"] *=
      ...
      steroidStats["R"]
        ...
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Talon" && steroidStats["R"] && rSkillPoint > 0
        ? 1.25 + 0.15 * rSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Taliyah" && steroidStats["P"]
        ? 1.1 + 0.05 * (level < 9 ? 0 : level < 12 ? 1 : level < 15 ? 3 : 6)
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Teemo" && steroidStats["W"] && wSkillPoint > 0
        ? 1.12 + 0.08 * wSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Trundle" && steroidStats["W"] && wSkillPoint > 0
        ? 1.12 + 0.08 * wSkillPoint
    ...
    obj["Move Speed"] *=
      nameChamp == "Udyr" && steroidStats["E"] && eSkillPoint > 0
        ? 1.23 + 0.07 * eSkillPoint
        : 1;
    obj["Move Speed"] *= nameChamp == "Viego" && steroidStats["P"] ? 1.1 : 1;
    obj["Move Speed"] *=
      nameChamp == "Viktor" && steroidStats["Q"] && steroidStats["P"] ? 1.3 : 1;
    obj["Move Speed"] *=
      nameChamp == "Vladimir" && steroidStats["Q"]
        ? 1.1 + (level < 6 ? 0 : level < 11 ? 0.1 : level < 16 ? 0.2 : 0.3)
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Vladimir" && steroidStats["W"] ? 1.375 : 1;
    obj["Move Speed"] *=
      nameChamp == "Volibear" && steroidStats["Q"] && qSkillPoint > 0
        ? 1.08 + 0.08 * qSkillPoint
        : 1;
    obj["Move Speed"] *=
      nameChamp == "Warwick" && steroidStats["W"] && wSkillPoint > 0
        ? 1.75 + 0.125 * wSkillPoint
        : 1;
    ...
    obj["Move Speed"] *=
      nameChamp == "Zoe" && steroidStats["W"] && wSkillPoint > 0
        ? 1.2 + 0.1 * wSkillPoint
        : 1;
    ...
    obj["Move Speed"] *=
      nameChamp == "Cassiopeia" && steroidStats["Q"] && qSkillPoint > 0
        ? 1 + 0.25 + 0.05 * qSkillPoint
        : 1;
    obj["Move Speed"] *=
      mainRune == "Fleet Footwork" && steroidStats["Runes"] ? 1.2 : 1;
    obj["Move Speed"] *=
      mainRune == "Predator" && steroidStats["Runes"]
        ? level < 8
          ? 1.25
          : 1.25 + 0.023 * (level - 7)
        : 1;
    obj["Move Speed"] *=
      mainRune == "Phase Rush" && steroidStats["Runes"]
        ? data["Melee?"] == 1
          ? level == 1
            ? 1.3
            : level == 2
        ...
        : 1;
    obj["Move Speed"] *=
      (mainFirstRune == "Nimbus Cloak" || secondFirstRune == "Nimbus Cloak") &&
      steroidStats["Runes"]
        ? 1.25
        : 1;
    obj["Move Speed"] *=
      mainThirdRune == "Celerity" || secondSecondRune == "Celerity" ? 1.1 : 1;
    obj["Move Speed"] *= 1 + 0.07 * gameStats["Cloud"];
    console.log(obj["Move Speed"]);
    obj["Move Speed"] *= bonusStats["Cloud"]
      ? 1.15 + (steroidStats["R"] ? 0.45 : 0)
      : 1;
    obj["Move Speed"] = obj["Move Speed"] - basicStatsChampion["Move Speed"];
    console.log(obj["Move Speed"]);
    // Critical %
    let bonusCrit =
      nameChamp == "Senna" ? Math.round(sennaStacks / 20) * 0.1 : 0;
    bonusCrit += nameChamp == "Tryndamere" && steroidStats["P"] ? 0.4 : 0;

    obj["Critical %"] =
      (itemStats["CC"] + bonusCrit) *
      ((nameChamp == "Yasuo" || nameChamp == "Yone") && steroidStats["P"]
        ? 2.5
        : 1);
    ...
    bonusLifeSteal +=
      ...
      steroidStats["E"] &&
      eSkillPoint > 0
        ? (-0.5 + 5.5 * eSkillPoint) / 100
        : 0;
    bonusLifeSteal +=
      nameChamp == "Nasus" ? (level < 7 ? 0.11 : level < 113 ? 0.16 : 0.21) : 0;
    bonusLifeSteal +=
      nameChamp == "Olaf"
        ? (8 * (level - 1) * (0.7025 + 0.0175 * (level - 1))) / 100
        : 0;
    bonusLifeSteal += nameChamp == "Sion" && steroidStats["P"] ? 1 : 0;
    bonusLifeSteal +=
      nameChamp == "Udyr" && steroidStats["W"] ? 0.14 + 0.01 * wSkillPoint : 0;
    bonusLifeSteal += 0.0035 * stackLegendBloodline;

    let selfHealing = hasSpiritVisage ? 1.25 : 1;
    selfHealing *=
      (mainThirdRune == "Revitalize" || secondSecondRune == "Revitalize") &&
      steroidStats["Runes"]
        ? 1.4
        : 1;
    ...
    bonusRegen +=
      nameChamp == "Singed" && steroidStats["R"] && rSkillPoint > 0
        ? -0.01 + 0.07 * rSkillPoint
        : 0;
    bonusRegen +=
      (mainSecondRune == "Second Wind" ||
        secondFirstRune == "Second Wind" ||
        secondSecondRune == "Second Wind") &&
      steroidStats["Runes"]
        ? 0.03
        : 0;
    bonusRegen +=
    ...
    obj["Hp Regen"] = itemStats["HP5"] + bonusRegen;
    obj["Hp Regen"] *= hasSpiritVisage ? 1.25 : 1;
    obj["Hp Regen"] *= nameChamp == "Trundle" && steroidStats["W"] ? 1.25 : 1;

    // Mana
    obj["Mana"] = itemStats["MP"] + runeStats["MP"];

    // Ability Power
    let bonusAP = hasArchangel ? 1 * Math.floor(obj["Mana"] * 0.01) : 0;
    bonusAP +=
      hasCrown && steroidStats["Items"]
        ? 10 + (level >= 9 ? 3 * (level - 8) : 0)
        : 0;
    bonusAP += hasCrown || hasRiftmaker ? 8 * nbLegendary : 0;
    bonusAP += hasDemonic ? Math.floor(obj["Hp"] * 0.02) : 0;
    bonusAP += hasEchoes ? 3 * Math.floor(0.25 * obj["Mana / Regen"]) : 0;
    bonusAP += hasEverfrost ? 10 * nbLegendary : 0;
    bonusAP += hasMejai ? 5 * stackMejai : 0;
    bonusAP += hasSeraph ? Math.floor(obj["Mana"] * 0.025) : 0;
    bonusAP +=
      hasFlowing && steroidStats["Items"] ? 3 + (15 / 17) * (level - 1) : 0;
    bonusAP +=
      nameChamp == "Singed" && steroidStats["R"] && rSkillPoint > 0
        ? -5 + 35 * rSkillPoint
        : 0;
    ...
    bonusAP +=
      mainRune == "Conqueror" && runeStats["ForceBit"] == 0
        ? (2 + 2.5 / (17 * (level - 1))) * stackConqueror
        : 0;
    bonusAP +=
      (secondFirstRune.includes("EyeBall") ||
        secondSecondRune.includes("EyeBall") ||
        mainSecondRune.includes("Eyeball")) &&
      runeStats["ForceBit"] == 0
        ? 2 * stackBounty + (stackBounty == 10 ? 10 : 0)
        : 0;
    bonusAP +=
      (secondFirstRune.includes("Poro") ||
        secondSecondRune.includes("Poro") ||
    ...
    let multiBonusAP = hasRabadon ? 1.4 : 1;
    multiBonusAP *= hasWardStone ? 1.2 : 1;
    multiBonusAP *= nameChamp == "Syndra" && steroidStats["P"] ? 1.15 : 1;
    multiBonusAP *= 1 + 0.05 * gameStats["Infernal"];
    obj["Ability Power"] =
      (bonusAP + itemStats["AP"] + runeStats["ForceBit"] == 0
        ? runeStats["Adaptive"]
        : 0) * multiBonusAP;

    // Range
    obj["Range"] = mainRune == "Lethal Tempo" && steroidStats["Runes"] ? 50 : 0;

    // Armor Penetration
    let armorFlat =
      nameChamp == "Aphelios" ? convertLethIntoArmorPen(5.5 * eSkillPoint) : 0;
    armorFlat +=
      hasYoumuu && steroidStats["Items"]
        ...
        : 0;
    armorFlat +=
      (mainFirstRune == "Sudden Impact" ||
        secondFirstRune == "Sudden Impact") &&
      steroidStats["Runes"]
        ? convertLethIntoArmorPen(9)
        : 0;
    armorFlat += convertLethIntoArmorPen(itemStats["LE"]);

    let bonusArmorPen =
      nameChamp == "Darius" && eSkillPoint > 0 ? 0.1 + 0.05 * eSkillPoint : 0;
    bonusArmorPen +=
      nameChamp == "Nilah" && qSkillPoint > 0
        ? 0.0033 * Math.floor(obj["Critical %"] * 100)
        : 0;
    bonusArmorPen += nameChamp == "Pantheon" ? 0.1 * rSkillPoint : 0;
    bonusArmorPen += nameChamp == "Yasuo" && steroidStats["R"] ? 0.5 : 0;
    bonusArmorPen += hasDivine ? 0.03 * nbLegendary : 0;
    bonusArmorPen += hasEclipse ? 0.04 * nbLegendary : 0;
    bonusArmorPen += hasRageblade ? 0.05 * nbLegendary : 0;
    bonusArmorPen += hasBlackCleaver && steroidStats["Items"] ? 0.3 : 0;
    bonusArmorPen += nameChamp == "Garen" && steroidStats["E"] ? 0.25 : 0;
    bonusArmorPen +=
      nameChamp.includes("Jarvan") && steroidStats["Q"] && qSkillPoint > 0
        ? 0.06 + 0.04 * qSkillPoint
        : 0;
    bonusArmorPen +=
      nameChamp == "Jayce" && steroidStats["R"] && rSkillPoint
        ? 0.1 + (level < 6 ? 0 : level < 11 ? 0.05 : level < 16 ? 0.1 : 0.15)
        : 0;
    bonusArmorPen += nameChamp == "Kayle" && steroidStats["Q"] ? 0.15 : 0;
    bonusArmorPen +=
      nameChamp.includes("Kog") &&
      nameChamp.includes("Maw") &&
      steroidStats["Q"] &&
      qSkillPoint > 0
        ? 0.21 + 0.02 * qSkillPoint
        : 0;
    bonusArmorPen +=
      nameChamp == "Nasus" && steroidStats["E"] && eSkillPoint > 0
        ? 0.2 + 0.05 * eSkillPoint
        : 0;
    bonusArmorPen += nameChamp == "Olaf" && steroidStats["Q"] ? 0.2 : 0;
    bonusArmorPen +=
      nameChamp == "Renekton" &&
      steroidStats["P"] &&
      steroidStats["E"] &&
      eSkillPoint > 0
        ? 0.225 + 0.025 * eSkillPoint
        : 0;
    bonusArmorPen += nameChamp == "Sion" && steroidStats["E"] ? 0.2 : 0;
    bonusArmorPen += nameChamp == "Trundle" && steroidStats["R"] ? 0.4 : 0;
    bonusArmorPen +=
      nameChamp == "Vi" && wSkillPoint > 0 && steroidStats["W"] ? 0.2 : 0;
    bonusArmorPen +=
      nameChamp == "Wukong" && qSkillPoint > 0 && steroidStats["Q"]
        ? 0.05 + 0.05 * qSkillPoint
        : 0;
    bonusArmorPen +=
      nameChamp == "Corki" && eSkillPoint > 0 && steroidStats["E"]
        ? 0.05 + 0.03 * eSkillPoint
        : 0;
    bonusArmorPen +=
      nameChamp == "Rengar" && rSkillPoint > 0 && steroidStats["R"]
        ? 0.06 + 0.06 * rSkillPoint
        : 0;

    obj["Armor Penetration"] =
      (enemyStats["Armor"] + enemyStats["Armor Bonus"]) * bonusArmorPen +
      armorFlat;

    // Magic Pen
    let flatMP = hasRocketbelt || hasLuden ? 5 * nbLegendary : 0;
    flatMP += hasShadowflame ? (10 + steroidStats["Items"] ? 10 : 0) : 0;
    flatMP +=
      (mainFirstRune == "Sudden Impact" ||
        secondFirstRune == "Sudden Impact") &&
      steroidStats["Runes"]
        ? 7
        : 0;
    flatMP += hasAbyssal ? 5 + Math.floor(obj["Hp"] * 0.0012) : 0;

    let bonusMP =
      nameChamp == "Mordekaiser" && eSkillPoint > 0
        ? 0.025 + 0.025 * eSkillPoint
        : 0;
    bonusMP += hasDivine ? 0.03 * nbLegendary : 0;
    bonusMP += hasRageblade ? 0.06 * nbLegendary : 0;
    bonusMP +=
      nameChamp == "Evelynn" && steroidStats["E"] && eSkillPoint > 0
        ? 0.325 + 0.025 * eSkillPoint
        : 0;
    bonusMP +=
      nameChamp == "Jayce" && steroidStats["R"] && rSkillPoint
        ? 0.1 + (level < 6 ? 0 : level < 11 ? 0.05 : level < 16 ? 0.1 : 0.15)
        : 0;
    bonusMP += nameChamp == "Karthus" && steroidStats["W"] ? 0.15 : 0;
    bonusMP += nameChamp == "Kayle" && steroidStats["Q"] ? 0.15 : 0;
    bonusMP +=
      nameChamp.includes("Kog") &&
      nameChamp.includes("Maw") &&
      steroidStats["Q"] &&
      qSkillPoint > 0
        ...
        : 0;
    bonusMP +=
      nameChamp == "Rumble" && steroidStats["E"] && eSkillPoint > 0
        ? 0.1 + 0.02 * eSkillPoint
        : 0;
    bonusMP += nameChamp == "Trundle" && steroidStats["R"] ? 0.4 : 0;
    bonusMP +=
      nameChamp == "Zoe" && steroidStats["E"] && eSkillPoint > 0
        ? 0.175 + 0.025 * eSkillPoint
        : 0;
    ...
    let bonusAH =
      ...
      hasShurelya
        ? 5 * nbLegendary
        : 0;
    bonusAH += hasGoredrinker || hasTrinity ? 3 * nbLegendary : 0;
    bonusAH += hasFlowing && steroidStats["Items"] ? 20 : 0;
    bonusAH += nameChamp == "Syndra" && rSkillPoint > 0 ? 10 * rSkillPoint : 0;
    bonusAH +=
      mainSecondRune == "Transcendence" ||
      secondFirstRune == "Transcendence" ||
      secondSecondRune == "Transcendence"
        ? level < 5
          ? 0
          : level < 8
          ? 5
    ...
    multi +=
      ...
      level >= 11
        ...
        : 0;
    obj["Ability Haste"] =
      (itemStats["AH"] + bonusAH + runeStats["AH"]) * multi;

    // SpellVamp
    let bonusSV =
      stackConqueror == 12 ? (data["Melee?"] == 1 ? 0.08 : 0.05) : 0;
    bonusSV +=
      nameChamp.includes("Lee") &&
      nameChamp.includes("Sin") &&
      steroidStats["E"] &&
      eSkillPoint > 0
        ? (-0.5 + 5.5 * eSkillPoint) / 100
        : 0;
    bonusSV += hasRiftmaker ? 0.02 * nbLegendary : 0;
    bonusSV +=
      nameChamp.includes("Sante") && steroidStats["Form"]
        ? 0.1 + 0.0075 * Math.floor(obj["Hp"] / 100)
        : 0;
    bonusSV +=
      nameChamp == "Aatrox" && eSkillPoint > 0
        ? 0.165 + 0.015 * eSkillPoint
        : 0;
    bonusSV +=
      nameChamp == "Fiddlesticks" && wSkillPoint > 0 && steroidStats["W"]
        ? 0.175 + 0.075 * wSkillPoint
        : 0;
    bonusSV +=
      nameChamp == "Hecarim" && wSkillPoint > 0 && steroidStats["W"]
        ? 0.25 + 0.02 * Math.floor(obj["Attack Damage"] / 100)
        : 0;
    bonusSV += nameChamp == "Morgana" ? 0.18 : 0;
    bonusSV +=
      nameChamp == "Kayn" && steroidStats["Form"]
        ? 0.2 + (0.1 / 17) * (level - 1)
        : 0;
    bonusSV += nameChamp == "Vladimir" && steroidStats["W"] ? 0.15 : 0;
    bonusSV +=
      nameChamp == "Warwick" && steroidStats["Q"] && qSkillPoint > 0
        ? 0.125 + 0.125 * qSkillPoint
        : 0;
    bonusSV += nameChamp == "Warwick" && steroidStats["R"] ? 1 : 0;

    obj["Spellvamp %"] = (itemStats["SV"] + bonusSV) * selfHealing;

    // Tenacity
    let bonusTenacity = hasIceborn ? 0.05 * nbLegendary : 0;
    bonusTenacity += hasSilvermere ? 0.5 : 0;
    bonusTenacity += hasSterak ? 0.3 : 0;
    bonusTenacity += nameChamp == "Garen" && steroidStats["W"] ? 0.6 : 0;
    bonusTenacity += nameChamp == "Milio" && steroidStats["R"] ? 0.65 : 0;
    bonusTenacity +=
      mainSecondRune == "Tenacity" ||
      secondFirstRune == "Tenacity" ||
      secondSecondRune == "Tenacity"
        ? 0.05 + 0.015 * stackLegendExceptBloodline
        : 0;
    bonusTenacity +=
      mainThirdRune == "Unflinching" || secondSecondRune == "Unflinching"
        ? 0.05 + (steroidStats["Runes"] ? 0.2 : 0)
        : 0;
    bonusTenacity += 0.06 * gameStats["Chemtech"];

    obj["Tenacity %"] = itemStats["TC"] / 100 + bonusTenacity;
    ...
    obj["Mana / Regen"] = itemStats["MP5"] + bonusMP5;
  ...
  useEffect(() => {
    ...
    objTotal["Crit"] =
      basicStatsChampion["Critical %"] + additionnalStats["Critical %"];
    objTotal["MP"] = basicStatsChampion["Mana"] + additionnalStats["Mana"];

    objTotal["AD"] =
      basicStatsChampion["Attack Damage"] + additionnalStats["Attack Damage"];
    objTotal["AD"] +=
      steroidStats["R"] && nameChamp == "Aatrox"
        ? rSkillPoint == 1
          ? objTotal["AD"] * 0.2
          : rSkillPoint == 2
          ? objTotal["AD"] * 0.325
        ...
        : 0;
    objTotal["AD"] +=
      nameChamp == "Olaf" && steroidStats["R"]
        ? 0 + 10 * rSkillPoint + 0.25 * objTotal["AD"]
        : 0;
    objTotal["AD"] +=
      nameChamp == "Naafiri" && steroidStats["R"]
        ? 10 +
          10 * (rSkillPoint - 1) +
          (0.1 + 0.05 * rSkillPoint) * objTotal["AD"]
        : 0;
    objTotal["AD"] +=
      nameChamp == "Trundle" && steroidStats["Q"]
        ? 0 + 20 * qSkillPoint + (0.5 + 0.1 * qSkillPoint) * objTotal["AD"]
        : 0;
    objTotal["AD"] +=
      nameChamp == "Jhin"
        ? 0.03 +
          (level <= 9
            ...
            : 0.04 * level) +
          0.003 * objTotal["Crit"] +
          0.025 * additionnalStats["Attack Speed %"]
        : 0;
    objTotal["AD"] += hasMuramana || hasManamune ? objTotal["MP"] * 0.025 : 0;

    objTotal["AH"] =
      basicStatsChampion["Ability Haste"] + additionnalStats["Ability Haste"];
    objTotal["AP"] =
      basicStatsChampion["Ability Power"] + additionnalStats["Ability Power"];
    objTotal["APenF"] =
      basicStatsChampion["Armor Penetration"] +
      additionnalStats["Armor Penetration"];
    objTotal["AR"] = basicStatsChampion["Armor"] + additionnalStats["Armor"];
    objTotal["AR"] +=
      nameChamp == "Gwen" && steroidStats["W"] && eSkillPoint > 0
        ? 15 + 2 * eSkillPoint + 0.07 * objTotal["AP"]
        : 0;
    objTotal["AR"] +=
      nameChamp == "Rammus" && wSkillPoint > 0
        ? 35 + ((30 + 10 * wSkillPoint) / 100) * objTotal["AR"]
        : 0;
    objTotal["AR"] +=
      nameChamp == "Taric" && wSkillPoint > 0
        ? ((9 * wSkillPoint) / 100) * objTotal["AR"]
        : 0;
    objTotal["AR"] +=
      nameChamp == "Malphite" && steroidStats["W"] && wSkillPoint > 0
        ? ((5 + 5 * eSkillPoint) / 100) * objTotal["AR"]
        : 0;

    objTotal["AS"] =
      basicStatsChampion["Attack Speed %"] + additionnalStats["Attack Speed %"];
    objTotal["AS"] +=
      nameChamp.includes("Renata") && steroidStats["W"] && wSkillPoint
        ? 0.1 + 0.1 * wSkillPoint + 0.02 * Math.floor(objTotal["AP"] / 100)
        : 0;
    objTotal["AS"] +=
      nameChamp.includes("Yuumi") && steroidStats["E"] && eSkillPoint
        ? 0.225 + 0.025 * eSkillPoint + 0.08 * Math.floor(objTotal["AP"] / 100)
        : 0;
    objTotal["AS"] +=
      nameChamp == "Kayle" && (steroidStats["P"] || level >= 16)
        ? 0.3 + 0.05 * Math.floor(objTotal["AP"] / 100)
        : 0;
    objTotal["AS"] +=
      nameChamp == "Volibear" && steroidStats["P"]
    ...
    objTotal["CritDMG"] =
      nameChamp == "Fiora" && steroidStats["E"] && eSkillPoint > 0
        ? 1.5 + 0.1 * eSkillPoint
        : objTotal["CritDMG"];
    objTotal["CritDMG"] *= nameChamp == "Shaco" && steroidStats["Q"] ? 1.4 : 1;
    objTotal["CritDMG"] *=
    ...
    objTotal["HitDmg"] = 1;
    objTotal["HPR"] =
      basicStatsChampion["Hp Regen"] + additionnalStats["Hp Regen"];
    objTotal["HPR"] +=
      (hasChalice || hasMikael || hasRedemption) && !hasEchoes
        ? 0.25 * Math.floor(objTotal["HPR"] / 0.25)
        : 0;
    objTotal["HPR"] +=
      hasWarmog && objTotal["HP"] >= 1100 ? 0.25 * objTotal["HP"] : 0;
    objTotal["HPR"] += nameChamp.includes("Mundo")
      ? 0.004 + 0.019 * (level - 1) * objTotal["HP"]
      : 0;
    objTotal["HPR"] +=
      nameChamp.includes("Mundo") && steroidStats["R"] && rSkillPoint > 0
        ? 0.02 * rSkillPoint * objTotal["HP"]
        : 0;

    objTotal["HPR"] +=
      nameChamp == "Garen" ? 0.015 + 0.086 * (level - 1) * objTotal["HP"] : 0;
    objTotal["HPR"] +=
      nameChamp == "Wukong" && steroidStats["P"] ? 0.021 * objTotal["HP"] : 0;

    objTotal["Leth"] =
      basicStatsChampion["Armor Penetration"] +
      additionnalStats["Armor Penetration"];
    objTotal["LS"] =
      basicStatsChampion["Lifesteal"] + additionnalStats["Lifesteal"];
    objTotal["LS"] +=
      nameChamp == "Senna" && additionnalStats["Critical %"] > 1
        ? 0.35 * (objTotal["Crit"] - 100)
        : 0;

    objTotal["HP"] = basicStatsChampion["Hp"] + additionnalStats["Hp"];
    objTotal["HP"] +=
      hasFimbulwinter || hasWinterApproach ? 0.08 * objTotal["MP"] : 0;
    objTotal["HP"] += hasHeartsteel ? 0.01 * nbLegendary * objTotal["HP"] : 0;
    objTotal["HP"] +=
      hasRadiant && steroidStats["R"] ? 0.125 * objTotal["HP"] : 0;
    ...
    objTotal["HP"] +=
      ...
      steroidStats["Form"]
        ? 50 +
          50 * rSkillPoint +
          1.2 * additionnalStats["Attack Damage"] +
          0.9 * objTotal["AP"]
        : 0;

    objTotal["MisHPV"] = 1;
    objTotal["MpenF"] =
      basicStatsChampion["Resist Penetration"] +
      additionnalStats["Resist Penetration"];
    objTotal["MPR"] =
      basicStatsChampion["Mana / Regen"] + additionnalStats["Mana / Regen"];
    objTotal["MR"] =
      basicStatsChampion["Magic Resist"] + additionnalStats["Magic Resist"];
    objTotal["MR"] +=
      nameChamp == "Gwen" && steroidStats["W"] && eSkillPoint > 0
        ? 15 + 2 * eSkillPoint + 0.07 * objTotal["AP"]
        : 0;
    objTotal["MR"] +=
      nameChamp == "Rammus" && wSkillPoint > 0
        ? 10 + ((25 + 5 * wSkillPoint) / 100) * objTotal["MR"]
        : 0;

    objTotal["MS"] =
      basicStatsChampion["Move Speed"] + additionnalStats["Move Speed"];
    objTotal["MS"] +=
      nameChamp == "Sion" && steroidStats["R"] ? 950 - objTotal["MS"] : 0;
    objTotal["MS"] *=
      nameChamp == "Akshan" && steroidStats["Q"]
        ? 1.4 + 0.05 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Lulu" && steroidStats["W"]
        ? 1.25 + 0.05 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Kayle" && steroidStats["W"] && wSkillPoint > 0
        ? 1.2 + 0.04 * wSkillPoint + 0.08 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Senna" && steroidStats["W"]
        ? 1.2 + 0.05 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Seraphine" && steroidStats["W"]
        ? 1.2 + 0.04 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp.includes("Renata") && steroidStats["W"] && wSkillPoint > 0
        ? 1.075 + 0.025 * wSkillPoint + 0.01 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Sona" && steroidStats["E"] && eSkillPoint
        ? 1.1 + 0.01 * eSkillPoint + 0.02 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Janna" && wSkillPoint > 0
        ? 1.05 + 0.01 * wSkillPoint + 0.02 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Lillia" && steroidStats["Q"] && qSkillPoint > 0
        ? 1.08 + 0.04 * qSkillPoint + 0.12 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Pyke" && steroidStats["W"]
        ? 1.4 + 0.015 * objTotal["Leth"]
        : 1;
    objTotal["MS"] *=
      nameChamp == "Shyvana" && steroidStats["W"] && wSkillPoint > 0
        ? 1.25 + 0.05 * wSkillPoint + 0.08 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Viego" && steroidStats["E"] && eSkillPoint > 0
        ? 1.225 + 0.025 * eSkillPoint + 0.04 * Math.floor(objTotal["AP"] / 100)
        : 1;
    objTotal["MS"] *=
      nameChamp == "Aphelios" &&
      apheliosStats["Main Weapon"] == "Severum, the Scythe Pistol" &&
      steroidStats["Q"]
    ...
    objTotal["Shield"] = 2;
    objTotal["TC"] =
      additionnalStats["Tenacity %"] + basicStatsChampion["Tenacity %"];
          ...
          <>
            <StatsTable
              stats={{
                ...
                Range: Math.floor(
                  (basicStatsChampion["Range"] + additionnalStats["Range"]) *
                    (hasRapidFireCanon ? 1.35 : 1)
                ),
            ...
            <div className="stats-table">
              ...
              <div className="stats-table-row">
                <table>
                  ...
                  {(nameMainRune == "Precision" ||
                    nameMainRune == "Domination" ||
                    secondRune == "Precision" ||
                    secondRune == "Domination") && (
                    <>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        ...
                        {(listLegend.includes(mainSecondRune) ||
                          listLegend.includes(secondFirstRune) ||
                          listLegend.includes(secondSecondRune)) && (
                          <tr>
                            <td>Legend</td>
                            <td>
                              <input
                        ...
                        {(mainSecondRune == "Bloodline" ||
                          secondFirstRune == "Bloodline" ||
                          secondSecondRune == "Bloodline") && (
                          <tr>
                        ...
                        {(listBounty.includes(mainThirdRune) ||
                          listBounty.includes(secondSecondRune)) && (
                          <tr>
                            <td>Bounty</td>
                            <td>
                              <input
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <select
                          value={enemyLevel}
                          onChange={handleChangeEnemyLevel}
                        >
                          {options}
                        </select>
                      </td>
                    ...
                    <tr>
                      <td>Hp</td>
                      <td>{enemyStats["Hp"] + enemyStats["Hp Bonus"]}</td>
                    </tr>
                    <tr>
                      <td>Armor</td>
                      <td>{enemyStats["Armor"] + enemyStats["Armor Bonus"]}</td>
                    </tr>
                    <tr>
                      <td>Magic Resist</td>
                      <td>
                        {enemyStats["Magic Resist"] +
                          enemyStats["Magic Resist Bonus"]}
                      </td>
                    </tr>
                    <tr>
                      <td>Current Hp</td>
                      <td>{enemyStats["Current Hp"]}</td>
                    </tr>
                    <tr>
                      <td>Current Hp %</td>
                      <td>{enemyStats["Current Hp %"].toFixed(3)}</td>
                    </tr>
                    <tr>
                      <td>Hp Bonus</td>
                      <td>
